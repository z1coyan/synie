...
run('PG 集成（角色菜单白名单）', () => {
  ...
  test('白名单读/sync 往返、幂等与审计留痕', async () => {
    ...
    const synced = await iam.syncRoleMenus(permit(ROLE_MENU_RESOURCE, 'update'), roleId, [menuB, menuA, menuA])
    expect(synced).toEqual([menuA, menuB].sort())
    expect(await iam.roleMenus(permit(ROLE_MENU_RESOURCE, 'read'), roleId)).toEqual([menuA, menuB].sort())
    ...
    // 幂等：同集合同步不产生新审计
    await iam.syncRoleMenus(permit(ROLE_MENU_RESOURCE, 'update'), roleId, [menuA, menuB])
    expect(await auditCount()).toBe(afterFirst)

    // 差量收缩：只留一个
    const shrunk = await iam.syncRoleMenus(permit(ROLE_MENU_RESOURCE, 'update'), roleId, [menuB])
    expect(shrunk).toEqual([menuB])
    expect(await auditCount()).toBe(afterFirst + 1)

    // 清空 = 恢复不限制
    expect(await iam.syncRoleMenus(permit(ROLE_MENU_RESOURCE, 'update'), roleId, [])).toEqual([])
  })

  test('目录外菜单码拒绝且逐个点名', async () => {
    const roleId = await mkRole('menus-unknown')
    const err = await iam
      .syncRoleMenus(permit(ROLE_MENU_RESOURCE, 'update'), roleId, [menuA, 'menu.sales.no-such-page', 'menu.bogus.x'])
      .catch((e) => e)
  ...
  test('内置角色 sync 抛冲突；删角色级联清白名单行', async () => {
    ...
    await iam.deleteRole(permit(ROLE_RESOURCE, 'delete'), roleId)
    const left = await db
  ...
  test('权限门控：无 sys.role_menu 码读/写均判 deny（403 只由码级判定产生）', async () => {
    // 服务层不再抛 forbidden：缺码在 decide 阶段即 deny，凭证根本签不出来
    expect(authz.decideFor(plainActor, ROLE_MENU_RESOURCE, 'read').outcome).toBe('deny')
    expect(authz.decideFor(plainActor, ROLE_MENU_RESOURCE, 'update').outcome).toBe('deny')
  })
  ...
  test('端点与 /auth/me：并集、停用排除、超管恒空、目录含 sys.role_menu', async () => {
    ...
    await iam.syncRoleMenus(permit(ROLE_MENU_RESOURCE, 'update'), roleA, [menuA, menuB])
    await iam.syncRoleMenus(permit(ROLE_MENU_RESOURCE, 'update'), roleB, [menuB, menuC])
