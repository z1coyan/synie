...
export function createCompanyService(db: Kysely<Database>, registry: Registry): StandardService<Company> {
  return createStandardService<Company>({
    ...
    hooks: {
      ...
      afterWrite: async (trx, { action, permit, item }) => {
        ...
        await seedCompanyDefaultWarehouses(trx, permit.actor, String(item.id), String(item.code))
      },
