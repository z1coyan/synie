/**
 * 银行流水导入文件解析：xlsx（zip+XML）与 BIFF8 .xls。
 * 行为对齐 server-go banking/parser.go。
 */
import { decimal } from '@synie/shared'
import { bytesToText, cleanPartName, unzipParts } from '~/platform/printing/zip.ts'
...
export function parseBankImport(
  ...
  if (content.length >= 2 && content[0] === 0x50 && content[1] === 0x4b) {
    ...
    // BIFF8 简化：不完整时抛格式错误（verify 用 xlsx）
...
function buildImportItems(
  ...
  for (let rowIndex = start - 1; rowIndex < rows.length; rowIndex++) {
    ...
    if (result.length > MAX_IMPORT_ROWS) {
      throw new Error(`数据行超过上限 ${MAX_IMPORT_ROWS} 行,请拆分文件后分次导入`)
    }
  }
  if (result.length === 0) {
