  ...
  const base = createStandardService<VatInvoice>({
    ...
    hooks: {
      ...
      beforeWrite: async (trx, { action, draft, before }) => {
        ...
        // 取号留在钩子（同事务）：values 口径与迁移前逐字一致——草稿无过账日，
        // 旧实现传「当下」冒充 posting_date；内核 numbering 只按 draft 派生 values
        if (action === 'create' && !String(draft.docNo ?? '').trim()) {
          draft.docNo = await numbering.nextInTx(trx, {
            resource: 'acc.vat_invoice',
            values: { company_id: draft.companyId, posting_date: new Date().toISOString() },
          })
        }
      },
