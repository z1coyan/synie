...
export function reconcileStatus(reconciled: ReturnType<typeof decimal>, amount: ReturnType<typeof decimal>): string {
...
export async function loadTransaction(db: DbHandle, id: string, lock: boolean): Promise<BankTransaction> {
  const rows = await sql<Record<string, unknown>>`
