...
func (s *Service) QueryBankImports(
	...
	result, err := listexec.List(ctx, listexec.Spec[BankImport]{
		Pool: s.pool, Resource: BankImportResourceMeta(), Label: "流水导入记录", Actor: actor,
		Source:       ` FROM acc_bank_import`,
		Select:       `SELECT ` + bankImportColumns,
		DefaultOrder: ` ORDER BY "id"`,
		Tiebreaker:   `, "id"`,
		Scan: func(rows pgx.Rows) (BankImport, error) {
			return scanBankImport(rows)
		},
	}, listQuery(query))
	if err != nil {
		return BankImportList{}, err
	}
	return BankImportList{Count: result.Count, Results: result.Results}, nil
}
