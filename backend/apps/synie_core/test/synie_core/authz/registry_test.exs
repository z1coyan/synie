defmodule SynieCore.Authz.RegistryTest do
  ...
  test "catalog 包含全部声明了权限前缀的资源" do
    ...
    assert prefixes ==
             Enum.sort(
               ~w(sys.user sys.role sys.role_permission sys.audit_log sys.file sys.storage sys.numbering_rule sys.print_template sys.setting base.company base.unit base.currency base.account base.market_instrument base.market_price sales.customer sales.order sales.delivery sales.reconciliation sales.quotation sales.setting purchase.supplier purchase.quotation purchase.order purchase.receipt purchase.outsourced_issue purchase.outsourced_receipt purchase.reconciliation hr.employee hr.attendance_punch hr.attendance_day hr.attendance_correction hr.payroll hr.payroll_payment hr.employee_loan inv.material_category inv.material inv.warehouse inv.stock_entry inv.stock_doc inv.stock_transfer inv.stock_count mfg.operation mfg.route_template mfg.bom mfg.demand mfg.work_order mfg.output mfg.setting acc.expense_report acc.gl_entry acc.gl_journal acc.bank_account acc.bank_transaction acc.bank_import_template acc.vat_invoice acc.bill acc.bill_transaction acc.bill_holding acc.setting)
             )
  end
