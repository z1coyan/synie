defmodule SynieCore.Purchase.OrderTest do
  ...
  setup do
    ...
    order = order!(%{company_id: company.id, party_type: :supplier, party_id: supplier.id})
  ...
  # 既有用例走零星单(自由录物料/价,不挂报价);常规单的报价派生见「订单分型」describe
  defp order!(attrs) do
    attrs =
      Map.merge(
        %{
  ...
  # 常规单 + 已审核固定价报价(同公司/供应商/币种,日期区间覆盖订单日期)
  ...
  test "已收数量默认 0 且不可手改,折算 base_qty 落库", ctx do
    item =
      item!(ctx.order, %{material_id: ctx.material.id, unit_id: ctx.box.id, qty: 2, price: 1})

    # 1 kg = 10 箱:2 箱 = 0.2 kg
    assert Decimal.equal?(item.base_qty, Decimal.new("0.2"))
    assert Decimal.equal?(item.received_qty, Decimal.new(0))

    [loaded] =
      OrderItem
      |> Ash.Query.filter(id == ^item.id)
      |> Ash.Query.load(:remaining_base_qty)
      |> Ash.read!(authorize?: false)

    assert Decimal.equal?(loaded.remaining_base_qty, Decimal.new("0.2"))
  end
  ...
  test "仅已审核单可关闭/作废,两终态均不可逆", ctx do
    ...
    # 另一单走作废
    order2 =
      order!(%{company_id: ctx.company.id, party_type: :supplier, party_id: ctx.supplier.id})
  ...
  describe "物料信息快照" do
    test "创建行落五个快照列(编号/名称/规格/客户料号/单位名称),采购不校验客户料约束", ctx do
      customer =
        SynieCore.Sales.Customer
        |> Ash.Changeset.for_create(:create, %{
          code: "C-#{System.unique_integer([:positive])}",
  ...
  describe "订单分型:常规行报价派生" do
    ...
    test "报价与订单公司/对手/币种不一致逐项被拒", ctx do
      ...
      other_party_quotation =
        quotation!(%{company_id: ctx.company.id, party_type: :supplier, party_id: supplier2.id})
  ...
  describe "订单分型:零星行" do
    test "数量超上限报错,改配置后按新值卡", ctx do
      ...
      assert Exception.message(error) =~ "零星条目数量超出上限"
      ...
      # 配置收紧到 5,按新值卡
      Setting.get()
      |> Ash.Changeset.for_update(:update, %{spot_item_max_qty: 5})
      |> Ash.update!(authorize?: false)
    ...
    test "零星行挂报价条目被拒", ctx do
      ...
      assert Exception.message(error) =~ "零星订单条目不可选择报价条目"
    end

    test "零星上限按行录入数量直接比,不做单位换算", ctx do
      # 箱是转换单位(1 kg = 10 箱):101 箱折默认单位仅 10.1 kg,仍按录入值 101 卡
      assert {:error, error} =
               OrderItem
               |> Ash.Changeset.for_create(:create, %{
                 order_id: ctx.order.id,
      ...
      assert item!(ctx.order, %{qty: 100, material_id: ctx.material.id, unit_id: ctx.box.id})
    end
  end

  describe "订单分型:头变更闸与类型锁死" do
    test "有条目改供应商/日期/币种报错,改备注/条款不拦", ctx do
      item!(ctx.order, %{material_id: ctx.material.id, unit_id: ctx.kg.id})
      ...
      assert {:error, error} =
               ctx.order
               |> Ash.Changeset.for_update(:update, %{party_id: supplier2.id})
               |> Ash.update(authorize?: false)
    ...
    test "无条目时改头关键字段不拦", ctx do
      ...
      updated =
        ctx.order
        |> Ash.Changeset.for_update(:update, %{party_id: supplier2.id, order_date: ~D[2026-07-18]})
        |> Ash.update!(authorize?: false)

      assert updated.party_id == supplier2.id
    end
    ...
    test "订单类型建后锁死,update 改型报错", ctx do
      ...
      updated =
        ctx.order
        |> Ash.Changeset.for_update(:update, %{order_type: :spot, remarks: "x"})
        |> Ash.update!(authorize?: false)

      assert updated.order_type == :spot
    end
  end
  ...
  describe "订单分型:审核复核" do
    ...
    test "零星配置收紧后,存量行审核被拒", ctx do
      item!(ctx.order, %{qty: 90, material_id: ctx.material.id, unit_id: ctx.kg.id})

      Setting.get()
      |> Ash.Changeset.for_update(:update, %{spot_item_max_qty: 50})
      |> Ash.update!(authorize?: false)
  ...
  describe "权限" do
    test "无 purchase.order 权限者创建被拒绝", ctx do
      user = user!()
      role = role!()
      assign!(user, role)
