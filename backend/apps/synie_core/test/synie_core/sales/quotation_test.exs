defmodule SynieCore.Sales.QuotationTest do
  ...
  alias SynieCore.Sales.{Customer, Quotation, QuotationItem, QuotationTier}
  ...
  setup do
    ...
    quotation =
      quotation!(%{company_id: company.id, party_type: :customer, party_id: customer.id})
  ...
  test "(物料, 单位) 单内唯一;同物料不同单位允许", ctx do
    ...
    # 另一张报价单不受影响
    other =
      quotation!(%{company_id: ctx.company.id, party_type: :customer, party_id: ctx.customer.id})
