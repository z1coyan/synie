defmodule SynieCore.Purchase.QuotationStatus do
  @moduledoc """
  采购报价单状态:草稿/已审核/已作废。无「已关闭」——报价到截止日天然失效,
  不存在「生效后提前终止履行」的概念;「已过期」是派生展示态(截止日 < 今天 且已审核),
  不落库、不跑定时任务翻状态。口径同销售报价单。
  """

  use Ash.Type.Enum, values: [draft: "草稿", audited: "已审核", voided: "已作废"]

  def graphql_type(_), do: :pur_quotation_status
end

defmodule SynieCore.Purchase.QuotationDraft do
...
defmodule SynieCore.Purchase.QuotationPartyType do
  @moduledoc "报价对手类型限供应商/内部公司(同采购订单);必填由属性兜底。"
...
defmodule SynieCore.Purchase.Quotation.HeadFieldsFrozen do
  @moduledoc """
  头关键字段变更闸:报价已有条目时,对手/公司/币种不可再改——
  改头会让既有行口径漂移,报错引导先删条目再改头。仅挂 update。
  (同销售报价单先例)
  """
  ...
  @impl true
  def validate(changeset, _opts, _context) do
    if head_changed?(changeset) and SynieCore.Purchase.Quotation.has_items?(changeset.data.id) do
...
defmodule SynieCore.Purchase.Quotation.DefaultCurrency do
  @moduledoc """
  报价单币种归一:币种留空默认单据公司本币。报价单没有金额(条目只有单价、无数量),
  因此不挂汇率、不做双币换算——将来报价转订单时汇率按当时行情填到订单头
  (同销售报价单先例)。公司读不到时跳过,由 CompanyAccessible 校验/外键兜底报错。
  """
...
defmodule SynieCore.Purchase.Quotation do
  @moduledoc """
  采购报价单(头),对应 `pur_quotation` 表。供应商/内部公司向本公司承诺价格的单据:
  纯价格承诺清单——条目只有单价没有数量,不构成数量/供货承诺(那是采购订单的事),
  也不派生 GL 分录、不动库存。结构全镜像销售报价单(ADR 2026-07-20-purchase-line)。
  ...
  报价单号全局唯一:留空按 `purchase.quotation` 编号规则自动取号(AutoNumber),
  手填原样保留。对手为多态引用(供应商/内部公司,无真外键,不能是单据公司自身)。
  币种挂头一单一币,默认公司本币;无汇率无双币(见 `DefaultCurrency`)。审核门槛:
  行数 ≥ 1,且每个数量梯度条目至少一个价格档。行见 `QuotationItem`,删除草稿时行与
  ...
  def grid_actions do
    [
      %{key: "audit", label: "审核", scope: "row", mutation: "auditPurQuotation", is_danger: false},
      %{key: "void", label: "作废", scope: "row", mutation: "voidPurQuotation", is_danger: true}
    ]
  end
  ...
  # 取值限供应商/内部公司(见 QuotationPartyType),与采购订单同一套 variants
  def poly_refs do
    %{
      party_id: %{
  ...
  actions do
    ...
    create :create do
      ...
      validate {SynieCore.Purchase.QuotationPartyType, []}
      validate {SynieCore.Acc.PartyExists, []}
      validate {SynieCore.Acc.PartyNotSelf, []}

      # 币种默认公司本币(无汇率,报价单不做双币)
      change {SynieCore.Purchase.Quotation.DefaultCurrency, []}
    ...
    update :update do
      ...
      validate {SynieCore.Purchase.QuotationDraft, []}
      validate {SynieCore.Purchase.Quotation.HeadFieldsFrozen, []}
      validate {SynieCore.Purchase.QuotationPartyType, []}
      validate {SynieCore.Acc.PartyExists, []}
      validate {SynieCore.Acc.PartyNotSelf, []}
  ...
  attributes do
    ...
    attribute :status, SynieCore.Purchase.QuotationStatus do
      allow_nil? false
      writable? false
      default :draft
      public? true
  ...
  relationships do
    ...
    has_many :items, SynieCore.Purchase.QuotationItem do
  ...
  def qty_tiered_without_tiers?(quotation_id) do
    ...
    |> Enum.any?(fn item ->
      not (SynieCore.Purchase.QuotationTier
