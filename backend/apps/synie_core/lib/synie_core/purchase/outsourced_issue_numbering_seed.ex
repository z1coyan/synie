defmodule SynieCore.Purchase.OutsourcedIssueNumberingSeed do
  @moduledoc """
  委外发料单编号规则种子(随建表迁移落地;老环境已完成初始化、向导不会再跑,
  新接 AutoNumber 的资源其启用规则须随迁移种子——先例 `SynieCore.Mfg.BomNumberingBackfill`)。

  种子 `purchase.outsourced_issue` 启用编号规则:`P(I)-发料日期(YYYYMMDD)-4 位补零序号`,
  ...
  @segments [
    ~s|{"type":"text","value":"P(I)-"}|,
    ~s|{"type":"field","field":"issue_date","format":"YYYYMMDD","label":"发料日期"}|,
    ~s|{"type":"text","value":"-"}|,
