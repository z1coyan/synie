defmodule SynieCore.Purchase.OrderStatus do
  @moduledoc "采购订单状态:草稿/已审核/已关闭/已作废。"

  use Ash.Type.Enum, values: [draft: "草稿", audited: "已审核", closed: "已关闭", voided: "已作废"]

  def graphql_type(_), do: :pur_order_status
end

defmodule SynieCore.Purchase.OrderType do
  @moduledoc "采购订单类型:常规订单(条目挂报价派生)/零星订单(自由录价,数量受 sal_setting 上限约束)。"

  use Ash.Type.Enum, values: [regular: "常规订单", spot: "零星订单"]

  def graphql_type(_), do: :pur_order_type
end

defmodule SynieCore.Purchase.OrderDraft do
...
defmodule SynieCore.Purchase.OrderTypeLocked do
...
defmodule SynieCore.Purchase.Order.HeadFieldsFrozen do
  ...
  @impl true
  def validate(changeset, _opts, _context) do
    if head_changed?(changeset) and SynieCore.Purchase.Order.has_items?(changeset.data.id) do
...
defmodule SynieCore.Purchase.Order.VerifyItems do
  @moduledoc """
  审核复核(第二道闸):审核前逐行重验分型规则,任一不满足即中止审核。
  常规订单每行:报价链接仍有效(已审核+订单日期在报价区间+公司/对手/币种一致,
  与行构建期同款判定,见 `Purchase.QuotationLink`)且单价与报价一致
  (固定价相等;梯度按行数量套档,低于首档起订量报错);
  零星订单每行:不得挂报价条目且数量不超 `sal_setting` 零星上限。
  ...
  alias SynieCore.Purchase.QuotationLink
  ...
  defp verify(%{order_type: :spot}, items) do
    max_qty =
      case SynieCore.Sales.Setting.get() do
        %{spot_item_max_qty: max} -> max
        _ -> nil
      end

    first_error(items, &verify_spot_item(&1, max_qty))
  end
...
defmodule SynieCore.Purchase.OrderPartyType do
  @moduledoc "采购订单对手类型限供应商/内部公司;必填由属性兜底。"
...
defmodule SynieCore.Purchase.Order.SyncCurrency do
  @moduledoc """
  订单头币种/汇率归一(口径同销售订单,见 ADR 2026-07-17-sales-order-currency):
...
defmodule SynieCore.Purchase.Order.RecalcItems do
  @moduledoc """
...
defmodule SynieCore.Purchase.Order.ClearItemDrawings do
  @moduledoc """
  ...
  def change(changeset, _opts, _context) do
    Ash.Changeset.before_action(changeset, fn cs ->
      cs.data.id
      |> item_ids()
      |> Enum.each(&SynieCore.Purchase.OrderItem.ClearDrawings.clear!/1)
...
defmodule SynieCore.Purchase.Order do
  ...
  纯业务承诺:审核不派生 GL 分录也不动库存;实物入库与未开票应付由采购入库单承载
  (`Purchase.Receipt`),开票仍由发票模块承载。结构全镜像销售订单
  (ADR 2026-07-20-purchase-line)。

  订单分型(`order_type`,新建时可选、建后锁死):常规订单条目必须挂有效采购报价条目,
  物料/单位/单价由报价派生;零星订单条目不挂报价、自由录价,数量受 `sal_setting`
  ...
  (AutoNumber),手填原样保留。对手为多态引用(供应商/内部公司,无真外键,
  不能是单据公司自身),审核业务门槛是行数 ≥ 1 且逐行通过分型复核。行见 `OrderItem`,
  删除草稿时行由 DB 级联删除(不走 OrderItem destroy 钩子,行的图纸挂接由
  ClearItemDrawings 在删单前显式清理)。

  双币:币种(原币)与汇率挂头,一单一币,留空默认公司本币;本币单汇率强制 1,
  外币单必填(SyncCurrency);草稿改币种/汇率同事务重算全部行本币列(RecalcItems),
  审核锁单即冻结。口径同销售订单(ADR 2026-07-17-sales-order-currency)。
  """
  ...
  def permission_label, do: "采购订单"
  def permission_actions, do: ~w(create read update delete audit close void)

  def grid_actions do
    [
      %{key: "audit", label: "审核", scope: "row", mutation: "auditPurOrder", is_danger: false},
      %{key: "close", label: "关闭", scope: "row", mutation: "closePurOrder", is_danger: false},
      %{key: "void", label: "作废", scope: "row", mutation: "voidPurOrder", is_danger: true}
    ]
  end

  # 对手是多态引用(party_type 判别、无 belongs_to),声明给 GridMeta 反射成多态 fk 列;
  # 取值限供应商/内部公司(见 OrderPartyType)
  def poly_refs do
    %{
      party_id: %{
  ...
  actions do
    ...
    create :create do
      ...
      change {SynieCore.Purchase.Order.SyncCurrency, []}
    ...
    update :update do
      ...
      change {SynieCore.Purchase.Order.RecalcItems, []}
    end
    ...
    destroy :destroy do
      ...
      change {SynieCore.Purchase.Order.ClearItemDrawings, []}
    end
    ...
    update :void do
      ...
      validate fn changeset, _context ->
        if __MODULE__.has_active_receipts?(changeset.data.id) do
          {:error, message: "订单存在已审核入库,请先作废相关采购入库单"}
        else
          :ok
        end
      end
  ...
  attributes do
    ...
    # 分型建后锁死(OrderTypeLocked):常规行挂报价派生,零星行自由录价受数量上限约束
    attribute :order_type, SynieCore.Purchase.OrderType do
      allow_nil? false
      default :regular
      public? true
      description "订单类型"
    ...
    # 金额链见 OrderItem.ComputeAmount(口径同销售订单)
    ...
    attribute :status, SynieCore.Purchase.OrderStatus do
      allow_nil? false
      writable? false
      default :draft
      public? true
  ...
  relationships do
    ...
    has_many :items, SynieCore.Purchase.OrderItem do
  ...
  @doc false
  # 存在已审核未作废的入库引用本订单任一条目时,订单不可作废
  def has_active_receipts?(order_id) do
