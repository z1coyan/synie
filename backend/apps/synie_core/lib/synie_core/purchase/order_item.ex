defmodule SynieCore.Purchase.OrderItem.SyncOrder do
  ...
  (同 Sales.OrderItem.SyncOrder 先例)
  """
...
defmodule SynieCore.Purchase.OrderItem.DeriveQuotation do
  @moduledoc """
  按订单分型建行(构建期,在 ComputeAmount/SnapshotMaterial 之前跑):

  常规订单——必须挂有效采购报价条目(报价单已审核+订单日期在报价区间+公司/对手/币种一致,
  与审核复核同款判定,见 `Purchase.QuotationLink`);物料/单位/单价由报价条目强制派生
  (忽略用户传值):固定价取报价单价,数量梯度按行数量套档(低于首档起订量无报价);
  税率用户未显式传时取报价条目税率;qty 变化(update)时重新套档派生 price。
  零星订单——不得挂报价条目,数量受 `sal_setting` 零星上限约束;
  ...
  def change(changeset, _opts, _context) do
    ...
    case read_order(order_id) do
      ...
      {:ok, %{order_type: :spot}} -> validate_spot(changeset)
      _ -> changeset
    end
  end
  ...
  defp validate_spot(changeset) do
    ...
    with %Decimal{} <- qty,
         %{spot_item_max_qty: max} <- SynieCore.Sales.Setting.get(),
         true <- Decimal.compare(qty, Decimal.new(max)) == :gt do
      Ash.Changeset.add_error(changeset, field: :qty, message: "零星条目数量超出上限(最大 #{max})")
    else
      _ -> changeset
    end
  end
...
defmodule SynieCore.Purchase.OrderItem.ComputeAmount do
  @moduledoc """
  金额链系统算(口径同销售订单,见 ADR 2026-07-17-sales-order-currency),四列均不允许手改
...
defmodule SynieCore.Purchase.OrderItem.SyncDrawings do
  @moduledoc """
  图纸挂接复制:行 create/update 后(after_action,在动作事务内)把物料当前
  drawing 槽位的 sys_file 集合同步为行的挂接(owner_type `pur_order_item`、
  ...
  alias SynieCore.Purchase.OrderItem.ClearDrawings
...
defmodule SynieCore.Purchase.OrderItem.ClearDrawings do
  @moduledoc """
  ...
  订单侧的清理见 SynieCore.Purchase.Order.ClearItemDrawings。
  """
  ...
  def clear!(item_id) do
    Attachment
    |> Ash.Query.filter(
      owner_type == "pur_order_item" and owner_id == ^item_id and category == "drawing"
    )
    |> Ash.read!(authorize?: false)
...
defmodule SynieCore.Purchase.OrderItem do
  @moduledoc """
  采购订单条目,对应 `pur_order_item` 表。
  ...
  按订单分型建行(见 `DeriveQuotation`):常规订单行必须挂有效采购报价条目,
  物料/单位/单价由报价强制派生(忽略用户传值);零星订单行不得挂报价条目、自由录价,
  数量受 `sal_setting` 零星上限约束。采购侧不校验客户物料约束,任何物料均可录入。
  物料编号/名称/规格/客户料号与单位名称是快照物理列:行保存(create/update)即按
  当前物料/单位重拍(共享 `Sales.SnapshotMaterial`),审核锁行即冻结,主数据后续变更不回溯;
  ...
  # 复用订单权限码;actions 为空不进权限目录(同 Sales.OrderItem 跟随 sales.order 的先例)
  ...
  actions do
    ...
    create :create do
      ...
      change {SynieCore.Purchase.OrderItem.SyncDrawings, []}
    end
    ...
    destroy :destroy do
      ...
      change {SynieCore.Purchase.OrderItem.SyncOrder, []}
      change {SynieCore.Purchase.OrderItem.ClearDrawings, []}
    end
    ...
    # 内部动作:采购入库审核/作废时加减已收数量(默认单位)。调用方须已 FOR UPDATE
    # 锁住本行,并完成超收校验;不注册 GraphQL。
  ...
  attributes do
    ...
    # 订购数量折默认单位(系统算,与库存/已收同口径);StockItemBaseQty 写入
    ...
    # 默认单位口径的已收累计:由采购入库审核/作废同步加减(投影列,第二期;
    # 见 ADR 2026-07-20-purchase-line)
  ...
  relationships do
    belongs_to :order, SynieCore.Purchase.Order do
      allow_nil? false
      public? true
    ...
    # 报价溯源链接:常规订单行必填(物料/单位/单价由此派生),零星订单行必须为空
    belongs_to :quotation_item, SynieCore.Purchase.QuotationItem do
      public? true
      attribute_public? true
      attribute_writable? true
      description "报价条目"
  ...
  calculations do
    ...
    calculate :order_status, SynieCore.Purchase.OrderStatus, expr(order.status) do
      public? true
      description "状态"
    end
    ...
    # 未收数量(默认单位)=订购 base − 已收;入库选条目筛 remaining_base_qty > 0
    calculate :remaining_base_qty, :decimal, expr(base_qty - received_qty) do
      public? true
      description "未收数量(物料默认单位)"
    end
  end
end
