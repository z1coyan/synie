defmodule SynieCore.Sales.QuotationTier.SyncItem do
  @moduledoc """
  ...
  def change(changeset, _opts, _context) do
    ...
    Ash.Changeset.before_action(changeset, fn cs ->
      with {:ok, %{item: item, quotation: quotation}} when not is_nil(item) <-
        ...
        case Ash.get(SynieCore.Sales.QuotationItem, item.id, authorize?: false) do
  ...
  defp load_item(item_id) do
    case SynieCore.Sales.QuotationItem
      ...
      {:ok, item} ->
        case SynieCore.Sales.Quotation
...
defmodule SynieCore.Sales.QuotationTier do
  @moduledoc """
  报价价格档,对应 `sal_quotation_tier` 表。数量梯度条目的起量价阶梯:
  ...
  `ClearTiersOnFixed`)。无独立权限点,动作复用 `sales.quotation` 权限码。
  """
  ...
  actions do
    ...
    create :create do
      ...
      change {SynieCore.Sales.QuotationTier.SyncItem, []}
  ...
  relationships do
    belongs_to :item, SynieCore.Sales.QuotationItem do
      allow_nil? false
      public? true
