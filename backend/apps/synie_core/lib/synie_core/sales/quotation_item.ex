defmodule SynieCore.Sales.QuotationPricingMode do
  @moduledoc """
  报价条目定价模式:固定价/数量梯度。枚举天然可扩值——将来的「期货梯度」
  只需加一值+独立档表,头与条目结构不动(ADR 2026-07-17-sales-quotation)。
  """

  use Ash.Type.Enum, values: [fixed: "固定价", qty_tiered: "数量梯度"]

  def graphql_type(_), do: :sal_quotation_pricing_mode
end

defmodule SynieCore.Sales.QuotationItem.SyncQuotation do
  @moduledoc """
  行与父报价单同步:报价单必须存在且草稿态(增删改行的前提);
  create 时把报价单 company_id 冗余到行(数据权限按公司过滤依赖此列)。
  构建期预检仅为友好报错;权威复检在 before_action 钩子内(事务内 FOR UPDATE
  持锁到提交,串行化行编辑与审核/作废)。同 OrderItem.SyncOrder 先例。
  """
...
defmodule SynieCore.Sales.QuotationItem.PricingRules do
  @moduledoc """
  定价模式一致性(ADR 2026-07-17-sales-quotation):
...
defmodule SynieCore.Sales.QuotationItem.ClearTiersOnFixed do
  @moduledoc """
...
defmodule SynieCore.Sales.QuotationItem do
  @moduledoc """
  销售报价条目,对应 `sal_quotation_item` 表。物料+单位粒度的价格承诺行:
  ...
  # 复用报价单权限码;actions 为空不进权限目录(同 OrderItem 跟随 sales.order 的先例)
  ...
  actions do
    ...
    update :update do
      ...
      change {SynieCore.Sales.SnapshotMaterial, []}
      # 梯度切回固定价时同事务清空价格档
      change {SynieCore.Sales.QuotationItem.ClearTiersOnFixed, []}
    end
  ...
  attributes do
    ...
    attribute :pricing_mode, SynieCore.Sales.QuotationPricingMode do
      allow_nil? false
      default :fixed
      public? true
      description "定价模式"
  ...
  relationships do
    belongs_to :quotation, SynieCore.Sales.Quotation do
      allow_nil? false
      public? true
    ...
    has_many :tiers, SynieCore.Sales.QuotationTier do
  ...
  calculations do
    ...
    # (同 OrderItem 先例);description 与 Quotation 对应属性保持一致
    ...
    calculate :quotation_status, SynieCore.Sales.QuotationStatus, expr(quotation.status) do
      public? true
      description "状态"
    end
