defmodule SynieCore.Sales.ReconciliationType do
  @moduledoc "销售对账单类型:常规/赠送样品(手选必填、保存后锁死,两类约束不对称,见 ADR 2026-07-21)。"

  use Ash.Type.Enum, values: [regular: "常规", gift_sample: "赠送/样品"]

  def graphql_type(_), do: :sal_reconciliation_type
end

defmodule SynieCore.Sales.ReconciliationStatus do
  @moduledoc "销售对账单状态:草稿/客户已确认/已结单/已作废(常规单无作废,赠送/样品单无确认)。"

  use Ash.Type.Enum,
    values: [draft: "草稿", confirmed: "客户已确认", closed: "已结单", voided: "已作废"]

  def graphql_type(_), do: :sal_reconciliation_status
end

defmodule SynieCore.Sales.ReconciliationDraft do
  ...
  def validate(changeset, _opts, _context) do
    if changeset.data.status == :draft do
      ...
      {:error, message: "仅草稿销售对账单可修改或删除"}
    end
  end
end

defmodule SynieCore.Sales.ReconciliationTypeLocked do
...
defmodule SynieCore.Sales.ReconciliationPartyType do
  @moduledoc "对账单对手类型限客户/内部公司(与销售订单/发货单一致)。"
...
defmodule SynieCore.Sales.Reconciliation.HeadFieldsFrozen do
  @moduledoc """
  头关键字段变更闸:对账单已有行时,对手不可再改——行上发货条目已锚定
  公司/对手/币种,改头会让既有行口径漂移(同 Delivery.HeadFieldsFrozen 先例)。仅挂 update。
  """
  ...
  @impl true
  def validate(changeset, _opts, _context) do
    if head_changed?(changeset) and SynieCore.Sales.Reconciliation.has_items?(changeset.data.id) do
...
defmodule SynieCore.Sales.Reconciliation.DebitAccountOk do
  @moduledoc """
  借方科目必填,须本公司、启用、非汇总(角色不限):常规单=发货贷方口径(收入/过渡),
  赠送/样品单=费用损失类手选——两型同一校验口径(同 Delivery.CreditAccountOk)。草稿保存即校验。
  """
...
defmodule SynieCore.Sales.Reconciliation.CreditAccountRole do
  @moduledoc """
  贷方科目必填,必须挂「未开票应收」角色(对冲发货时挂的未开票应收);
  另须本公司、启用、非汇总(同 Delivery.DebitAccountRole)。草稿保存即校验。
  """
...
defmodule SynieCore.Sales.Reconciliation.FillDefaultAccounts do
  @moduledoc """
  建单按「公司默认过账科目」整组代入借贷科目(可改):对账单贷方(未开票应收)←
  默认发货借方,对账单借方(发货贷方口径)← 默认发货贷方;无默认则留空,由必填校验兜底。
  ...
  def change(changeset, _opts, _context) do
    ...
    case company_id && SynieCore.Sales.CompanyAccountDefault.get_for_company(company_id) do
      ...
      defaults ->
        changeset
        |> fill_if_blank(:debit_account_id, defaults.delivery_credit_account_id)
        |> fill_if_blank(:credit_account_id, defaults.delivery_debit_account_id)
    end
  end
...
defmodule SynieCore.Sales.Reconciliation do
  @moduledoc """
  销售对账单(头),对应 `sal_reconciliation` 表。发货与开票之间的勾稽层:
  自身确认/结单不过往来重分类——常规单(draft→confirmed→closed)的
  「未开票应收→应收账款」迁移由发票审核关联触发(五笔分录,见 `SynieCore.Acc.VatInvoice`);
  赠送/样品单(draft→closed)审核即结单并过账 借费用损失/贷未开票应收,
  已结单可作废(回滚分录组与已对账数量)。详见 docs/adr/2026-07-21-sales-reconciliation.md。

  草稿不占量;数量消耗发生在生效时点(常规单 confirm、赠送/样品单 audit),
  逐行校验剩余可对账量并累加发货条目 `reconciled_qty`,撤回/作废回滚。
  单号全局唯一,留空按 `sales.reconciliation` 编号规则取号。行见 `ReconciliationItem`。
  """
  ...
  def grid_actions do
    [
      ...
      %{
        ...
        label: "撤回确认",
        scope: "row",
        mutation: "unconfirmSalReconciliation",
        is_danger: true
      },
      %{
        key: "audit",
      ...
      %{
        ...
        label: "作废",
        scope: "row",
        mutation: "voidSalReconciliation",
        is_danger: true
      }
    ]
  end
  ...
  actions do
    ...
    create :create do
      ...
      validate {SynieCore.Sales.ReconciliationPartyType, []}
      validate {SynieCore.Acc.PartyExists, []}
      validate {SynieCore.Acc.PartyNotSelf, []}

      # 默认科目代入须先于必填校验(声明序即执行序,同 AutoNumber 须在构建期取号)
      change {SynieCore.Sales.Reconciliation.FillDefaultAccounts, []}

      validate {SynieCore.Sales.Reconciliation.DebitAccountOk, []}
      validate {SynieCore.Sales.Reconciliation.CreditAccountRole, []}
    ...
    update :update do
      ...
      validate {SynieCore.Sales.ReconciliationDraft, []}
      validate {SynieCore.Sales.ReconciliationTypeLocked, []}
      validate {SynieCore.Sales.Reconciliation.HeadFieldsFrozen, []}
      validate {SynieCore.Sales.ReconciliationPartyType, []}
      validate {SynieCore.Acc.PartyExists, []}
      validate {SynieCore.Acc.PartyNotSelf, []}
      validate {SynieCore.Sales.Reconciliation.DebitAccountOk, []}
      ...
      change fn changeset, _context ->
        Ash.Changeset.before_action(changeset, fn cs ->
          case __MODULE__.lock_reconciliation(cs.data.id) do
            {:ok, %{status: :draft}} -> cs
            _ -> Ash.Changeset.add_error(cs, message: "仅草稿销售对账单可修改或删除")
          end
        end)
      end
    end
    ...
    # 客户确认(仅常规单):生效时点,逐行校验剩余可对账量并累加已对账数量
    update :confirm do
      accept []
      require_atomic? false
      ...
      validate fn changeset, _context ->
        cond do
          changeset.data.reconciliation_type != :regular ->
            {:error, message: "仅常规对账单可客户确认"}

          changeset.data.status != :draft ->
            {:error, message: "仅草稿销售对账单可客户确认"}
    ...
    # 结单审核(仅赠送/样品单):审核即结单,消耗数量并过账 借费用损失/贷未开票应收
    update :audit do
      accept [:posting_date]
      require_atomic? false
    ...
    # 内部动作:发票作废/红冲时把常规单退回客户已确认(同事务,不注册 GraphQL)
  ...
  attributes do
    ...
    attribute :reconciliation_type, SynieCore.Sales.ReconciliationType do
      allow_nil? false
      public? true
    ...
    attribute :status, SynieCore.Sales.ReconciliationStatus do
      allow_nil? false
      writable? false
      default :draft
      public? true
  ...
  relationships do
    ...
    belongs_to :debit_account, SynieCore.Base.Account do
      ...
      attribute_public? true
      attribute_writable? true
      description "借方科目(常规单=发货贷方口径;赠送/样品单=费用损失类;草稿必填)"
    end
    ...
    has_many :items, SynieCore.Sales.ReconciliationItem do
  ...
  def linked_invoice?(reconciliation_id) do
    ...
    |> Ash.Query.filter(sal_reconciliation_id == ^reconciliation_id)
    |> Ash.exists?(authorize?: false)
  end

  @doc false
  # 生效消耗:按发货条目分组,逐条 FOR UPDATE 锁行、复核剩余可对账量后累加
  ...
  @doc false
  # 赠送/样品单结单过账:借单头借方(费用损失)/贷单头贷方(未开票应收,往来须带对手),
  # 金额=勾选条目价税合计(本币);为零跳过分录(同发货先例,但科目仍草稿必填)
        ...
        case SynieCore.Acc.GL.validate_entries(reconciliation.company_id, entries) do
          :ok ->
            SynieCore.Acc.GL.post!(
              %{
                voucher_type: "sales.reconciliation",
  ...
  def cancel_gift_gl!(reconciliation) do
    ...
    SynieCore.Acc.GL.cancel!("sales.reconciliation", reconciliation.id)
    :ok
  end
  ...
  # 发票作废/红冲:解除关联后把常规单退回客户已确认(在发票动作事务内调用)
  @doc false
  ...
  defp adjust_reconciled(items, direction) do
    items
    |> Enum.group_by(& &1.delivery_item_id)
    |> Enum.reduce_while(:ok, fn {delivery_item_id, group}, :ok ->
      delta =
        group
        |> Enum.map(& &1.base_qty)
        |> Enum.reduce(Decimal.new(0), &Decimal.add/2)
      ...
      delivery_item =
        ...
        |> Ash.Query.filter(id == ^delivery_item_id)
        |> Ash.Query.lock("FOR UPDATE")
        |> Ash.read_one!(authorize?: false)

      with :ok <- check_remaining(delivery_item, group, delta, direction),
           :ok <- do_adjust(delivery_item, delta, direction) do
        {:cont, :ok}
      else
  ...
  # 生效方向才校验剩余:剩余可对账 = 发货 base − 已对账(回退方向只由非负约束兜底)
  defp check_remaining(delivery_item, group, delta, :add) do
  ...
  defp check_remaining(_delivery_item, _group, _delta, :sub), do: :ok

  defp do_adjust(delivery_item, delta, direction) do
