defmodule Mix.Tasks.Synie.Demo do
  ...
  - 示例:覆盖全业务链——6 客户、6 供应商、17 物料、4 员工,销/采报价各 5 张、订单各 4 张、
    发货/入库各 3 张、对账各 2 张(各 1 张已确认并被发票结单)、期初入库/出库/调拨/盘点、
    工序/工艺模板/BOM,以及银行账户与流水、手工凭证、报销单、工资单与销/进项发票各 1 张
  ...
  **仅允许** `MIX_ENV=dev` 或 `test`。已初始化的库直接拒绝(不会覆盖)。
  ...
  def run(_args) do
    ...
    Mix.shell().info([
      ...
      "* 完成。",
      :reset,
      " 登录 admin / admin123,公司 JT,已含全业务链示例数据(客商/物料/单据/财务)。"
    ])
  end
end
