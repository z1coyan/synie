...
const SAMPLE_DEFAULTS = {
  ...
  language: 'zh-CN' as SetupLanguage,
  template: 'SMALL' as AccountTemplate,
} as const
...
function StepCompany(props: { path: SetupPath; onDone: () => void }) {
  ...
  const [template, setTemplate] = useState<AccountTemplate | null>(sample ? SAMPLE_DEFAULTS.template : null)
...
function StepLanguage(props: { seedSampleData: boolean }) {
  ...
  const complete = useMutation({
    mutationFn: () => completeSetup(language, props.seedSampleData),
    onSuccess: () => {
