...
import { OrderDrawerProvider } from './sales-orders/-order-drawer'
  ...
  return (
    <OrderDrawerProvider>
      ...
      <Tabs
        ...
        onSelectionChange={(key) => navigate({ to: `/scm/sales-orders/${String(key)}` })}
        className="mt-4"
      >
        <Tabs.ListContainer>
          ...
          <Tabs.List aria-label="销售订单视图" className="w-fit min-w-0 *:w-auto">
            {TABS.map((t) => (
              <Tabs.Tab
                key={t.id}
                id={t.id}
                render={(domProps) => <Link {...(domProps as object)} to={`/scm/sales-orders/${t.id}`} />}
              >
                {t.label}
                <Tabs.Indicator />
              </Tabs.Tab>
