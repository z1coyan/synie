...
export function DeliveryDrawerProvider({ children }: { children: ReactNode }) {
  ...
  const openDrawer = useCallback<OpenDeliveryDrawer>((mode, delivery) => {
    ...
    gqlFetch<{ salDeliveryItems: { results: Row[] } }>(FETCH_ITEMS, {
      ...
      .catch((e) => {
        if (my !== reqIdRef.current) return
        toast.danger('发货条目加载失败', { description: (e as Error).message })
        setItems([])
        setItemsSnapshot([])
      })
  }, [])
                      ...
                      if (id && (row?.materialId == null || row?.unitId == null)) {
                        try {
                          const data = await gqlFetch<{
                            salOrderItems: { results: Row[] }
                          }>(
                            `query ($id: ID!) {
                              salOrderItems(filter: {id: {eq: $id}}, limit: 1, offset: 0) {
                                results {
                                  id materialId unitId materialCode materialName materialSpec
                                  customerPartNo unitName qty baseQty shippedQty remainingBaseQty
                                  order { id orderNo }
                                }
                              }
                            }`,
                          ...
                          row = data.salOrderItems.results[0] ?? row
                        } catch {
