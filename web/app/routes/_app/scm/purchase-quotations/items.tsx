...
import {
  ...
  useQuotationDrawer,
  type OpenQuotationDrawer,
} from './-quotation-drawer'
import { isExpired } from './quotations'

export const Route = createFileRoute('/_app/scm/purchase-quotations/items')({
...
function buildOverrides(openDrawer: OpenQuotationDrawer) {
  return {
    // 卡片:物料作标题、供应商作副标题、状态/单价/截止日期作摘要
    companyId: { mobileRole: 'hide' },
    // 物料列:全站统一富单元格(快照四字段,编号点开物料速览);报价条目无图纸挂接,不传 drawingOwnerType
    materialCode: {
      label: '物料',
      mobileRole: 'title',
...
function QuotationItemsTab() {
  ...
  const { requestAudit, auditDialog } = useAuditDoc(purchaseQuotationAuditConfig)
  ...
  return (
    <>
      <SynieDataGrid
        ...
        // purQuotationItems 复用 purchase.quotation 权限码,meta capabilities 为空:显式声明本视图
