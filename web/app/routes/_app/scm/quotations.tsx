...
import { QuotationDrawerProvider } from './quotations/-quotation-drawer'

export const Route = createFileRoute('/_app/scm/quotations')({
  component: QuotationsLayout,
})

// 销售报价两视图一页承载(照销售订单 tabs 先例):报价条目(行级明细,默认视图)、
...
function QuotationsLayout() {
  ...
  const selected = TABS.find((t) => pathname.includes(`/scm/quotations/${t.id}`))?.id ?? 'items'
  ...
  return (
    <QuotationDrawerProvider>
      ...
      <Tabs
        ...
        onSelectionChange={(key) => navigate({ to: `/scm/quotations/${String(key)}` })}
        className="mt-4"
      >
        <Tabs.ListContainer>
          {/* 默认 min-w-full + tab w-full 满宽平分;收紧为内容宽靠左,容器全宽底边保留 */}
          <Tabs.List aria-label="销售报价视图" className="w-fit min-w-0 *:w-auto">
            {TABS.map((t) => (
              <Tabs.Tab
                key={t.id}
                id={t.id}
                render={(domProps) => <Link {...(domProps as object)} to={`/scm/quotations/${t.id}`} />}
              >
                {t.label}
                <Tabs.Indicator />
              </Tabs.Tab>
