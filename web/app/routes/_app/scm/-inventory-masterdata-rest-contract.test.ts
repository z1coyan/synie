...
describe('库存四主数据 REST 迁移契约', () => {
  ...
  test('跨业务页持续复用库存 REST client，履约页面同步切换 REST', () => {
    ...
    expect(setup).toContain('companyClient.create')
    expect(setup).not.toContain('seedWarehouseDefaults')
    ...
    expect(otherStock).toContain('fetchMyPermissions')
    expect(otherStock).toContain('hasPermission')
