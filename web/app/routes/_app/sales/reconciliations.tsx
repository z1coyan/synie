...
function SalesReconciliationsLayout() {
  ...
  const selected =
    TABS.find((t) => pathname.includes(`/sales/reconciliations/${t.id}`))?.id ?? 'items'
    ...
    <ReconciliationDrawerProvider urlSync>
      ...
      <Tabs
        ...
        onSelectionChange={(key) => {
          if (key !== selected) navigate({ to: `/sales/reconciliations/${String(key)}` })
        }}
        className="mt-4"
      >
        <Tabs.ListContainer>
          <Tabs.List aria-label="销售对账视图" className="w-fit min-w-0 *:w-auto">
            {TABS.map((t) => (
              <Tabs.Tab
                key={t.id}
                id={t.id}
                href={`/sales/reconciliations/${t.id}`}
              >
                {t.label}
                <Tabs.Indicator />
              </Tabs.Tab>
