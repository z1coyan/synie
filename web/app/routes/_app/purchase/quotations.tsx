...
function PurchaseQuotationsLayout() {
  ...
  const selected = TABS.find((t) => pathname.includes(`/purchase/quotations/${t.id}`))?.id ?? 'items'

  return (
    <QuotationDrawerProvider urlSync>
      <h1 className="font-brand text-xl">采购报价</h1>
      ...
      <Tabs
        ...
        onSelectionChange={(key) => {
          if (key !== selected) navigate({ to: `/purchase/quotations/${String(key)}` })
        }}
        className="mt-2"
      >
        <Tabs.ListContainer>
