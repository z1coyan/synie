...
import type { PurchaseReturnSavedDraft } from '~/lib/resources/returns'
...
const purchaseReturnBinding = resourceBindingFor('purReturns')
const purchaseReturnDraft = aggregateDraftFor('purReturns')
  ...
  columns: [
    {
      key: 'materialName',
      label: '物料',
      render: auditMaterialCell({ drawingOwnerType: 'pur_return_item' }),
    },
...
/**
 * 新建态:公司选定/变更时整组覆盖借贷科目为该公司默认(入库槽反转代入,同采购对账先例;
 * 无默认则清空)。编辑态公司锁死,不重灌。
 */
    ...
    <div className="mt-6 grid grid-cols-1 gap-4 border-t border-separator pt-4 lg:grid-cols-2">
      ...
      <div>
        <RemoteSelect
          resource="basAccounts"
          label="贷方科目"
          placeholder={companyId ? '选择贷方科目…' : '先选择公司'}
          value={credit}
...
/**
 * 可退货入库条目固定筛选(弹窗 SynieDataGrid fixedFilter):
 * 1. 入库单已审核未作废 2. 公司/对手与退货头一致 3. 剩余可退数量 > 0
 * 使用 REST FilterState，权限与公司/对手/剩余数量条件由服务端白名单解释。
 */
function receiptItemGridFilter(values: Record<string, unknown>): FilterState | null {
...
function receiptItemDisplay(r: Row): string {
  ...
  return [material || '入库条目', rem].filter(Boolean).join(' · ')
}
...
/** 行形态判别:未挂入库条目即手工行(手填物料/价税) */
function isManualItem(vals: Record<string, unknown>): boolean {
  return vals.receiptItemId == null || vals.receiptItemId === ''
}
...
export function ReturnDrawerProvider({
  ...
  // 入库条目缓存:选择时写入完整行,transformItem 带出快照名
  ...
  const savedDraftsRef = useRef(new Map<string, PurchaseReturnSavedDraft>())
  // 单据抽屉骨架:双态状态机、URL 身份→整单草稿装载(竞态安全)、深链补拉全部收口进 hook
  const drawer = useDocumentDrawer<PurchaseReturnSavedDraft>({
    resource: 'purReturns',
    urlSync,
    loadDraft: (id) => {
  ...
  // 预热入库条目缓存并写 draftHeadRef(onSubmit 合并头字段用)
  useEffect(() => {
    const saved = drawer.draft
    draftHeadRef.current = saved ?? {}
    const itemRows = saved?.items ?? []
    // 编辑态预热缓存:存量行不必再点选入库条目也能过校验/回填
    const cache = new Map<string, Row>()
    for (const r of itemRows) {
      if (r.receiptItemId != null) {
        cache.set(String(r.receiptItemId), {
          id: String(r.receiptItemId),
  ...
  const acceptSavedDraft = (saved: PurchaseReturnSavedDraft) => {
    setDraftErrors({})
  ...
  const baseCfg = drawerConfig('purReturns')
  const drawerCfg = {
    ...baseCfg,
    fields: {
      ...baseCfg.fields,
    ...
    <ReturnDrawerOpenProvider value={openDrawer}>
      ...
      <SynieRecordDrawer
        ...
        extraContent={(mode, row, values, patchValues) => {
          ...
          const diGridFilter = receiptItemGridFilter(values)

          // 条目录入两种方式:「从入库单导入」(弹窗选入库条目,锁定回填物料/单位快照)
          // 与「手工加行」(不选入库条目,手填物料/单位/数量/含税单价/税率)
          const itemFields: Record<string, FieldOverride> = {
            idx: { visible: () => false },
            receiptItemId: {
              order: 0,
              section: '方式一:从入库单导入(源单行)',
              label: '入库条目(留空保存即为手工行)',
              input: ({ value, onChange, isDisabled, patchValues: patchItem }) => (
                <RemoteDialogSelect
                  resource="purReceiptItems"
                  label="入库条目"
                  dialogTitle="选择可退货入库条目"
                  placeholder={
                    diGridFilter ? '点击选择入库条目;留空则按下方手工填写建行…' : '先选齐公司与对手'
                  }
                  ...
                  onChange={(id, ditem) => {
                    if (id && ditem) receiptItemsRef.current.set(String(id), ditem)
                    onChange(id)
                    // 物料/单位随入库条目锁定带出;collectValues 会丢 hidden 字段,
            ...
            // 手工行字段组(未挂入库条目时可见):物料/单位/含税单价/税率手填
            materialId: {
              order: 1,
            ...
            // 物料信息只读回显(源单行;不进提交手改路径;值由入库条目 patch 写入)
            materialName: {
              order: 1,
              label: '物料',
              visible: (v) => !isManualItem(v),
              ...
              <SynieEditableTable
                ...
                overrides={{
                  receiptItemId: {
                    ...
                    render: (_v, r) =>
                      r.orderNo != null && r.orderNo !== ''
                        ? String(r.orderNo)
                        : r.receiptItemId == null || r.receiptItemId === ''
                          ? '手工行'
                          : undefined,
                  },
                  ...
                  materialName: {
                    ...
                    render: materialCellRender({ drawingOwnerType: 'pur_return_item' }),
                  },
                  unitName: { label: '单位' },
                  baseQty: { label: '折算数量' },
                  remarks: { label: '行备注' },
                ...
                validateItem={(vals, _items, editing) => {
                  if (isManualItem(vals)) {
                    ...
                    const cached = receiptItemsRef.current.get(String(vals.receiptItemId))
                    const materialId =
                      cached?.materialId ?? editing?.materialId ?? vals.materialId
                    const unitId = cached?.unitId ?? editing?.unitId ?? vals.unitId
                    if (!materialId || !unitId) return '请重新选择入库条目以带出物料'
                  }
                ...
                transformItem={(vals, editing) => {
                  ...
                  return {
                    ...
                    receiptItemId: manual ? null : vals.receiptItemId,
                    // 新建行预填头默认仓
        ...
        onSubmit={async (values, mode) => {
          ...
          if (writerBag?.create || writerBag?.update) {
            throw new Error('采购退货表单不得暴露 RecordWriter create/update')
          }
          const request = buildPurchaseReturnDraft(
            { ...draftHeadRef.current, ...values },
            items,
          )
          setDraftErrors({})
          ...
          try {
            ...
            await Promise.all([
              ...
              resourceBindingFor('purReturnItems').cache.invalidateGrid(
                queryClient,
              ),
              resourceBindingFor('purReceiptItems').cache.invalidateGrid(
                queryClient,
              ),
            ...
            toast.success(`采购退货单已${mode === 'create' ? '创建' : '更新'}`)
