...
export async function fetchSetupStatus(): Promise<SetupStatus> {
  ...
  const status = await apiData<SetupStatus>(api.setup.status.$get())
...
export function createSetupFirstUser(input: SetupFirstUserInput) {
  return apiData<SetupFirstUserResult>(
...
export function seedSetupCommonCurrencies() {
  return apiData<{ created: number }>(api.setup.currencies['seed-common'].$post())
}

/** 仅启用选定本位币，其余预置币保持停用 */
export function activateSetupBaseCurrency(currencyId: string) {
  return apiData<{ success: boolean }>(
