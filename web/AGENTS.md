...
- 字段行为（必填/只读 `edit`/条件显隐 `visible`/栅格 `cols`/默认值/分组标题 `section`/字段前插槽 `before`）通过 `fields` override 声明，提交 mutation 写在页面 `onSubmit` 回调；接入范例见 `routes/_app/system/roles.tsx`。
