...
实现终审(2026-07-13)留下的跟进项:

- 补测:GL (0,0) 默认路径拒绝与 DB CHECK 级用例;发票 blockers「gross>0」「税非负」分支;3 处 assert_raise 锚定 message
- amount.ts:≥10^16 加护栏、-0.001 负零先判、跨组补零逻辑加注释
- parseItems 数组分支逐行 try/catch(照 numbering 过滤坏行保留其余)
- mirrorInvoiceId edit 态渲染为 fk 链接(组件能力扩展)
- 「顺手审核」分支不可达(create 不收 posting_date):二选一——create 表单放开过账日期或删分支
- 批量操作失败 toast 透出后端错误消息(use-grid-actions 共享组件,全站受益)
- 发票抽屉 items 可从 rowId 自查行数据初始化,省一次 FETCH_ITEMS 请求
- GL 红字迁移 down 在产生红字行后不可执行,迁移注释注明
- gl.ex moduledoc 写明 reverse! 不自带锁、靠调用方(单据动作 FOR UPDATE)串行化的契约;GlJournal 接入红冲时同样要「仅 audited + FOR UPDATE」
- 发票表单 companyId 变更时若对手类型为内部公司一并清 partyId(现靠后端 PartyNotSelf 兜底,体验润色)
- RecordDrawer `meta` prop 的 JSDoc 注明「需为稳定引用」(内联字面量会复现渲染循环风险)
