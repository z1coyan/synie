...
- 防重录:custom unique index `(company_id, invoice_code, invoice_no)` **NULLS NOT DISTINCT**(数电票 code 为空也参与判重)**WHERE invoice_no IS NOT NULL**(草稿无号不占坑);需 PG15+。
...
后端:PartyType 扩展、GL 红字扩展、`acc_vat_invoice` 资源(动作/权限/审计/编号/校验)+ 迁移 + 测试;前端:组件本地 meta 扩展、金额/大写工具、发票页(表格+抽屉+三动作弹窗)+ 标签菜单注册。
...
1. **红冲=红字负数分录,需放宽 GL 核心 CHECK**(影响 acc_gl_entry 表约束与 GL.post! 校验,默认参数下既有行为不变)。若不想动 GL,退路是借贷对调正数凭证(发生额虚增,不推荐)。
2. **明细遵你所选用 json 存**,代价是扩 EditableTable/RecordDrawer「本地 meta」模式;若你更看重行级校验或近期就要关联物料,应改独立子表(切换主要是后端建表,前端反而省扩展)。
3. **发票类型枚举六值**(专/普/电子专/电子普/数电专/数电普)是否覆盖你的业务;增值税普通发票(卷票)等归入 normal。
4. **「审核人」按系统审核人处理**(audited_by 自动记 actor);发票纸面的开票人/复核人/收款人是三个文本字段。
5. 数电票无发票代码:invoice_code 可空,防重录唯一索引用 NULLS NOT DISTINCT + partial,需 Postgres 15+(synie-pg 容器版本需在实施首任务确认,不满足则退化为 code 空串规约)。
