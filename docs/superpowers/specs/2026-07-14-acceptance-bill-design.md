...
实现终审(2026-07-14)留下的跟进项:

- 贴现联动的 edit 态与陈旧值一并修:选段控件编辑草稿时不回填(pickedHolding 空导致改利率不重算)、occurredOn 变更利息不重算、清空金额显示 0——同族问题,一次修完(提交侧注入自洽,均为显示层)
- 非接收类型传 bill_attrs 应拒(现 API 直调 endorse+bill_attrs 会静默建档顶掉 bill_id;前端不触发)
- holdings 的 CompanyScope 读过滤补一条专测(现模式照抄 bank_transaction,用例全走 authorize?: false)
- 自定义控件必填星标统一补(companyId/bankAccountId/numberInput 三处,校验由 missingRequired 兜住,纯视觉)
- 票号查档体验:无 acc.bill:read 权限时失焦弹红 toast;跨公司已建档票查档不命中、票面被「挂接不覆盖」静默丢弃,可在 create 成功后提示「票号已建档,票面以库内为准」
- 兑付段无终态:同段兑付后再录接收引擎会放行(重放时段已移除),现依赖录入纪律;随「商票拒付」轮一并建模
- 批量动作 Toast 透出逐条报错(use-grid-actions 平台级既有跟进项,作废批量失败现只报计数)
- dev/test 的 `config :ash_postgres, upsert_with_merge?: false`(共享容器 PG16 vs min_pg_version 17)待容器升级后删除并验证 MERGE 路径
