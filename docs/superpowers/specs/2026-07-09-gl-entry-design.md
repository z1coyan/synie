...
- 分录只追加、不可改:用户没有任何直接写入口;作废单据 = 该单据分录标记 `is_cancelled`,消费侧(报表/前端)显式过滤;"read 默认过滤 + include_cancelled 参数"留跟进。跨期更正走业务层红字凭证,对 GL 层是普通分录。
...
- 前端凭证录入页与分录查询页
- 分录 read 的 `include_cancelled` 参数与默认过滤
- `GL.post!` 防重复过账守卫(同 voucher 已存在未作废分录则拒绝)
- `GlJournal` 补 `has_many :entries` 反向导航
- "validate 通过但 post! 抛错"的事务回滚直测与并发路径直测
- GL 模块入参形状(Decimal/键)防御与文档化
