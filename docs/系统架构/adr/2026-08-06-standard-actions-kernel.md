...
制造域主数据（工序/工艺模板/BOM 头与子行/模具设计读删）已回归派生（2026-08-21）：路由层 `standardRoutes`/`standardChildRoutes` 新增 `guards` 端点级覆盖（封闭词表内的 allOf/anyOf 组合子按端点声明），整域弹射收回为逐动作弹射；BOM create（status 双写形+建后启用）与模具 create/update（跨实体写面）按动作弹射保留手写（同挂载点先注册，同路径先注册胜出）。
