...
`server/src/platform/posting/` 是跨域单据共享层（过账骨架 `skeleton.ts` / 形状登记 `shapes.ts`）。
聚合内核 W0 领域基元亦落此目录扩容（决策见 `docs/migration/aggregate-kernel-decisions.md` D10），**不**新建 `platform/domain-primitives/`：
