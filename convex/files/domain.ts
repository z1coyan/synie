...
export const createUploadIntent = authedMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const id = await ctx.db.insert('uploadIntents', {
      ...
      category: args.category?.trim() || 'default',
      companyId,
      status: 'pending',
...
export const listAttachments = authedQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const rows = args.category
      ? await ctx.db.query('attachments').withIndex('by_owner_category', (q) =>
          q.eq('ownerType', args.ownerType).eq('ownerId', args.ownerId).eq('category', args.category!),
        ).take(200)
      : await ctx.db.query('attachments').withIndex('by_owner', (q) =>
...
export const attach = authedMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const category = args.category?.trim() || 'default'
    const existing = await ctx.db.query('attachments').withIndex('by_file', (q) => q.eq('fileId', file._id)).collect()
    const duplicate = existing.find((row) => row.ownerType === args.ownerType && row.ownerId === args.ownerId && row.category === category)
...
export const removeAttachment = authedMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!row) return null
    await resolveOwner(ctx, ctx.actor, row.ownerType, row.ownerId)
  ...
  handler: async (ctx, args) => {
    ...
    if (intent.ownerType && intent.ownerId) {
      const attachmentId = await ctx.db.insert('attachments', {
        ...
        category: intent.category, companyId: intent.companyId, insertedAt,
      })
      attachment = { id: attachmentId, fileId, ownerType: intent.ownerType, ownerId: intent.ownerId, category: intent.category, companyId: intent.companyId, insertedAt: new Date(insertedAt).toISOString() }
    }
    await ctx.db.patch(intent._id, { status: 'finalized', fileId })
    return { file: presentFile((await ctx.db.get(fileId))!), attachment }
  },
})
...
export const authorizeDownload = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const file = await ctx.db.get(args.fileId)
    if (!file || file.status !== 'ready') throw synieError('not_found', '文件不存在')
    if (file.uploadedById === actor.userId) return file
    const attachments = await ctx.db.query('attachments').withIndex('by_file', (q) => q.eq('fileId', file._id)).collect()
    for (const attachment of attachments) {
      try {
        await resolveOwner(ctx, actor, attachment.ownerType, attachment.ownerId)
        return file
      } catch {
        // Try another visible attachment without exposing hidden owner existence.
      }
    }
