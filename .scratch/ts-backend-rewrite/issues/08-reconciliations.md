...
- 2026-07-28 isolation worktree（grok-4.5 fix-08-11）：重读实现与契约，无占位；`tsc --noEmit` 绿；PG 对账 7 项全绿；`:18121` `verify-supply-reconciliation-rest` → `ok meta=12 permissionFirst=41 sides=2 actions=8 defaults=1 nullSlots=4 noDelete=1 orderFlowOR=4 stringIds=1 candidateOrderType=2 graphql=0 cleanup=0`。无代码修复。
